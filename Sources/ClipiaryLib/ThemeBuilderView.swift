...
// MARK: - Dependent Group

/// A `[`-shaped bracket: vertical line with short horizontal caps at top and bottom.
private struct BracketShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        // top cap
        p.addRect(CGRect(x: 0, y: 0, width: capWidth, height: lineWidth))
        // bottom cap
...
/// A `[`-bracket on the left visually groups the dependent children.
private struct DependentGroup<Content: View>: View {
    let enabled: Bool
