...
## Debugging pasteboard types

To inspect what an app puts on the clipboard (useful for verifying password manager integration), copy something and run:

```sh
/usr/bin/swift tools/pb_debug.swift
```

This prints all pasteboard types and checks for `org.nspasteboard.ConcealedType` and `org.nspasteboard.AutoGeneratedType`. Note: use `/usr/bin/swift` explicitly to avoid PATH conflicts with the Python `swift` package.

## Debugging update UI
Press `CTRL+U` to cycle through "fake" update phases to test the update UI in absence of real updates.
